...
setwd("~/Google Drive/GitHub/article1")
...
make_plot_results_simul = function(data_results, maxdepth, max_w_mod, plot_title = "", criteria, data_ranks = NULL){
  ...
  if (is.null(data_ranks)){
    plot = ggplot(data = to_plot, aes(x = model, y = col_criteria,
      ...
      scale_shape_discrete("Parameters (q, R2_C)",
...
make_scatter_plot_results_simul = function(data_results, maxdepth, max_w_mod,
                                           prop_censored, target_R2_C,
                                           plot_title = "", criteria){
  to_plot1 = data_results[(data_results$prop_censored == prop_censored &
                             data_results$target_R2_C == target_R2_C &
                             data_results$algo != "w_gam"), ]
...
make_rank_simul = function(data_results, minleaf, maxdepth, max_w_mod,
                           criteria, minimise){
  ...
  if (minimise){
    to_plot = to_plot %>% group_by(prop_censored, target_R2_C, iter) %>% mutate(rank_model = rank(col_criteria, ties.method = 'first'))
  } else {
    to_plot = to_plot %>% group_by(prop_censored, target_R2_C, iter) %>% mutate(rank_model = rank(-col_criteria, ties.method = 'first'))
  }

  results = to_plot %>%
    group_by(prop_censored, target_R2_C, model) %>%
...
res_simul_weibull_log_1000_rank =
  make_rank_simul(data_results = res_simul_weibull_log_1000,
                  maxdepth = 4,
                  minleaf = 50,
                  max_w_mod = 50,
                  criteria = "criteria_non_censored.mse",
                  minimise = T)

res_simul_weibull_identite_1000_rank =
  make_rank_simul(data_results = res_simul_weibull_identite_1000,
                  maxdepth = 4,
                  minleaf = 50,
                  max_w_mod = 50,
                  criteria = "criteria_non_censored.mse",
...
res_simul_mix_dep_log_1000_rank =
  make_rank_simul(data_results = res_simul_mix_dep_log_1000,
                  maxdepth = 4,
                  minleaf = 50,
                  max_w_mod = 50,
                  criteria = "criteria_non_censored.mse",
...
res_simul_mix_dep_identite_1000_rank =
  make_rank_simul(data_results = res_simul_mix_dep_identite_1000,
                  maxdepth = 4,
                  minleaf = 50,
                  max_w_mod = 50,
                  criteria = "criteria_non_censored.mse",
...
res_simul_mix_indep_log_1000_rank =
  make_rank_simul(data_results = res_simul_mix_indep_log_1000,
                  maxdepth = 4,
                  minleaf = 50,
                  max_w_mod = 50,
                  criteria = "criteria_non_censored.mse",
...
res_simul_mix_indep_identite_1000_rank =
  make_rank_simul(data_results = res_simul_mix_indep_identite_1000,
                  maxdepth = 4,
                  minleaf = 50,
                  max_w_mod = 50,
                  criteria = "criteria_non_censored.mse",
...
table_rank = cbind(model_T = rep(x = c("weibull","mix_dep", "mix_indep"), times = c(120,120,120)),
                   phi = rep( rep(x = c("log","identity"), times = c(60,60)), 3),
                   rbind(res_simul_weibull_log_1000_rank,
                         res_simul_weibull_identite_1000_rank,
                         res_simul_mix_dep_log_1000_rank,
                         res_simul_mix_dep_identite_1000_rank,
                         res_simul_mix_indep_log_1000_rank,
...
table_rank2 = dcast(data = table_rank,
                    formula = model_T + phi + prop_censored + target_R2_C ~ model, value.var = "mean_rank")

xtable(table_rank2, digits = 1)

# si l'on veut un tableau avec le rang moyen sur les 6 cas R2_C * prop_censored

table_rank2_summary = table_rank2 %>% group_by(model_T, phi) %>%
  summarise(RF = mean(RF),
            wRF1_KM = mean(wRF1_KM),
            wRF1_Cox = mean(wRF1_Cox),
            wRF1_RSF = mean(wRF1_RSF),
            wRF2_KM = mean(wRF2_KM),
            wRF2_Cox = mean(wRF2_Cox),
            wRF3_KM = mean(wRF3_KM),
            wRF3_Cox = mean(wRF3_Cox),
            rsf_reg = mean(rsf_reg),
            cox_reg = mean(cox_reg))

data.frame(table_rank2_summary)
...
plot1 = make_plot_results_simul(data_results = res_simul_weibull_identite_1000_summary,
                                ...
                                criteria = "mean_mse"
                                #,data_ranks = res_simul_weibull_identite_1000_rank
                                )
plot2 = make_plot_results_simul(data_results = res_simul_mix_indep_identite_1000_summary,
                                ...
                                criteria = "mean_mse"
                                #data_ranks = res_simul_mix_indep_identite_1000_rank
                                )
plot3 = make_plot_results_simul(data_results = res_simul_mix_dep_identite_1000_summary,
                                ...
                                criteria = "mean_mse"
                                #data_ranks = res_simul_mix_dep_identite_1000_rank
                                )
plot4 = make_plot_results_simul(data_results = res_simul_weibull_log_1000_summary,
                                ...
                                criteria = "mean_mse"
                                #data_ranks = res_simul_weibull_log_1000_rank
                                )
plot5 = make_plot_results_simul(data_results = res_simul_mix_indep_log_1000_summary,
                                ...
                                criteria = "mean_mse"
                                #data_ranks = res_simul_mix_indep_log_1000_rank
                                )
plot6 = make_plot_results_simul(data_results = res_simul_mix_dep_log_1000_summary,
                                ...
                                criteria = "mean_mse"
                                #data_ranks = res_simul_mix_dep_log_1000_rank
                                )
...
res_simul_weibull_log_5000 = read.csv("output_2017-10-22/res_simul_weibull_log_5000.csv")
res_simul_weibull_log_1000 = read.csv("output_2017-10-21/res_simul_weibull_log_1000.csv")
res_simul_weibull_identite_5000 = read.csv("output_2017-10-22/res_simul_weibull_identite_5000.csv")
res_simul_weibull_identite_1000 = read.csv("output_2017-10-21/res_simul_weibull_identite_1000.csv")



plot_indiv1 = make_scatter_plot_results_simul(data_results = res_simul_weibull_log_1000,
                                              maxdepth = 4,
                                              max_w_mod = 50,
                                              prop_censored = 0.5,
                                              target_R2_C = 0.1,
                                              plot_title = "weibull_log_1000, target_R2_C = 0.5, prop_censored = 0.5",
                                              criteria = "criteria_non_censored.mse")

plot_indiv2 = make_scatter_plot_results_simul(data_results = res_simul_weibull_log_5000,
                                              maxdepth = 5,
                                              max_w_mod = 50,
                                              prop_censored = 0.5,
                                              target_R2_C = 0.1,
                                              plot_title = "weibull_log_5000, target_R2_C = 0.5, prop_censored = 0.5",
                                              criteria = "criteria_non_censored.mse")

plot_indiv3 = make_scatter_plot_results_simul(data_results = res_simul_weibull_identite_1000,
                                              maxdepth = 4,
                                              max_w_mod = 50,
                                              prop_censored = 0.5,
                                              target_R2_C = 0.1,
                                              plot_title = "weibull_identite_1000, target_R2_C = 0.5, prop_censored = 0.5",
                                              criteria = "criteria_non_censored.mse")

plot_indiv4 = make_scatter_plot_results_simul(data_results = res_simul_weibull_identite_5000,
                                              maxdepth = 5,
                                              max_w_mod = 50,
                                              prop_censored = 0.5,
                                              target_R2_C = 0.1,
                                              plot_title = "weibull_identite_5000, target_R2_C = 0.5, prop_censored = 0.5",
                                              criteria = "criteria_non_censored.mse")


multiplot(plot_indiv1$boxplot,
          plot_indiv2$boxplot + ylim(c(1.3, 1.65)),
          plot_indiv1$scatter_plot,
          plot_indiv2$scatter_plot + ylim(c(1.3, 1.65)),
          cols = 2)

multiplot(plot_indiv3$boxplot,
          plot_indiv4$boxplot,
          plot_indiv3$scatter_plot,
          plot_indiv4$scatter_plot,
          cols = 2)
