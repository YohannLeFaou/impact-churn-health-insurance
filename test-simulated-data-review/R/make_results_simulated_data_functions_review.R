...
generate_censored_data = function(n_simul,
  ...
  ## computation of theoretical weigths of the observations (with threshold)
  Te_tresh = pmin(Te,censoring_threshold)
  delta_tresh = Te_tresh <= C
  w_theo = 1 / n_simul * delta_tresh / exp(-(Te_tresh / (lambdaC * exp(-coefC)))^shapeC)
                     ...
                     res = foreach(k=1:n_repet,
                                   ...
                                   .verbose = F) %dopar% {
                                     ...
                                     list_data = generate_censored_data(n_simul = grid_param$n_simul[i],
                                                                        ...
                                                                        mixture_T_type = switch(is.na(grid_param$mixture_T_type[i]) + 1, as.character(grid_param$mixture_T_type[i]), NULL),
                                     ...
                                     # add theoretical weights to mat_w
                                     w_theo_train = list_data$w_theo[train_lines]
                                     w_theo_train_thres1000 = pmin(w_theo_train, min(w_theo_train[w_theo_train > 0]) * 1000)
                                     w_theo_train_thres1000 = w_theo_train_thres1000 / sum(w_theo_train_thres1000)

                                     w_theo_test = list_data$w_theo[-train_lines]
                                     w_theo_test_thres1000 = pmin(w_theo_test, min(w_theo_test[w_theo_test > 0]) * 1000)
                                     w_theo_test_thres1000 = w_theo_test_thres1000 / sum(w_theo_test_thres1000)

                                     mat_w_theo = matrix(data = c(w_theo_train_thres1000, w_theo_test_thres1000), ncol = 1)
                                     colnames(mat_w_theo) = "theo"
                                     mat_w = cbind(mat_w, mat_w_theo)
